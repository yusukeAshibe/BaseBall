...
@Service
@Transactional
public class ScrapeEaglesService {
	@Autowired
	...
	public void scrapePitcher() {
		...
		try {
			document = Jsoup.connect("https://baseball.yahoo.co.jp/npb/teams/376/memberlist?kind=p").get();
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public void scrapeBatter() {
		...
		try {
			document = Jsoup.connect("https://baseball.yahoo.co.jp/npb/teams/376/memberlist?kind=b").get();
		} catch (IOException e) {
			e.printStackTrace();
		}
